...
hsv = []; color_tile9 = []; color_tile25 = []; color_state = []
...
def draw_tile(tile, image, sticker_name):
    for i in range(tile):
        hsv.append(image[stickers[sticker_name][i][1] + 10][stickers[sticker_name][i][0] + 10])
...
def main():
    ...
    RESET_SURF, RESET_RECT = makeText('Reset',    TEXTCOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 90)
    NEW_SURF,   NEW_RECT   = makeText('New Game', TEXTCOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 60)
    SOLVE_SURF, SOLVE_RECT = makeText('Solve',    TEXTCOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 30)

    mainBoard, solutionSeq = generateNewPuzzle(1)
    SOLVEDBOARD = getSolvedBoard() # a solved board is the same as the board in a start state.
...
def getStartingBoard():
    ...
    board = []; column = []; counter = 1
    for i in range(1,(BOARDHEIGHT * BOARDWIDTH) + 1):
        if(color_tile25[i-1] == 'black'): column.append(BLANK)
        else: column.append(counter); counter += 1
        if(i%5 == 0): board.append(column); column = []
    return board

def getSolvedBoard():
    solvedBoard = []; solvedColumn = []; solvedcounter = 1
    for i in range(1,(BOARDHEIGHT * BOARDWIDTH) + 1):
        solvedColumn.append(solvedcounter); solvedcounter += 1
        if(i%5 == 0): solvedBoard.append(solvedColumn); solvedColumn = []
...
def drawTile(tilex, tiley, number, adjx=0, adjy=0):
    ...
    left, top = getLeftTopOfTile(tilex, tiley)
    if flag_status == 0:
        pygame.draw.rect(DISPLAYSURF, color_tile25[number - 1], (left + adjx, top + adjy, TILESIZE, TILESIZE))
    elif flag_status == 1:
        pygame.draw.rect(DISPLAYSURF, color_tile25[number], (left + adjx, top + adjy, TILESIZE, TILESIZE))
